 ...
 * @returns {object}
 */
export function GameRender(scope: Game) {
    return function render() {
        if (CONFIG_CONSTANTS.DEBUG && ErrorHandlerInstance.data.ctx) {
            const ctx = ErrorHandlerInstance.data.ctx,
        ...
        for (var menu in menus) {
            ...
            if (m && m.activated === true) {
                if (m.spawned === true && m.transitionSpawn) {
                    m.spawned = false;
                    // TransitionEffectFade(scope, m.transitionSpawnDuration); //TODO
